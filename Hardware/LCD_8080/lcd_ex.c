...
void lcd_ex_st6201_reginit()
{
    //---------Start Initial Code ------//
    lcd_wr_regno( 0xff );
    lcd_wr_data( 0xa5 );
    lcd_wr_regno( 0xE7 );  // TE_output_en
    lcd_wr_data( 0x10 );
    lcd_wr_regno( 0x35 );  // TE_ interface_en
    lcd_wr_data( 0x00 );   // 01
    lcd_wr_regno( 0x36 );
    lcd_wr_data( 0xC0 );  // C8
    lcd_wr_regno( 0x3A );
    ...
    lcd_wr_regno( 0x44 );  // VBP
    lcd_wr_data( 0x15 );
    lcd_wr_regno( 0x45 );  // VFP
    lcd_wr_data( 0x15 );
    lcd_wr_regno( 0x7d );  // vdds_trim[2:0]
    lcd_wr_data( 0x03 );

    lcd_wr_regno( 0xc1 );  // avdd_clp_en avdd_clp[1:0] avcl_clp_en avcl_clp[1:0]
    lcd_wr_data( 0xbb );   // 0xbb 88 a2
    lcd_wr_regno( 0xc2 );  // vgl_clp_en vgl_clp[2:0]
    lcd_wr_data( 0x05 );
    lcd_wr_regno( 0xc3 );  // vgl_clp_en vgl_clp[2:0]
    lcd_wr_data( 0x10 );
    lcd_wr_regno( 0xc6 );  // avdd_ratio_sel avcl_ratio_sel vgh_ratio_sel[1:0] vgl_ratio_sel[1:0]
    lcd_wr_data( 0x3e );
    lcd_wr_regno( 0xc7 );  // mv_clk_sel[1:0] avdd_clk_sel[1:0] avcl_clk_sel[1:0]
    lcd_wr_data( 0x25 );
    lcd_wr_regno( 0xc8 );  // VGL_CLK_sel
    lcd_wr_data( 0x21 );
    lcd_wr_regno( 0x7a );  // user_vgsp
    lcd_wr_data( 0x51 );   // 58
    lcd_wr_regno( 0x6f );  // user_gvdd
    lcd_wr_data( 0x49 );   // 4F
    lcd_wr_regno( 0x78 );  // user_gvcl
    lcd_wr_data( 0x57 );   // 70
    lcd_wr_regno( 0xc9 );
    lcd_wr_data( 0x00 );
    lcd_wr_regno( 0x67 );
    lcd_wr_data( 0x11 );
    // gate_ed
    lcd_wr_regno( 0x51 );  // gate_st_o[7:0]
    lcd_wr_data( 0x0a );
    lcd_wr_regno( 0x52 );  // gate_ed_o[7:0]
    lcd_wr_data( 0x7D );   // 7A
    lcd_wr_regno( 0x53 );  // gate_st_e[7:0]
    lcd_wr_data( 0x0a );
    lcd_wr_regno( 0x54 );  // gate_ed_e[7:0]
    lcd_wr_data( 0x7D );   // 7A
    // sorce
    lcd_wr_regno( 0x46 );  // fsm_hbp_o[5:0]
    lcd_wr_data( 0x0a );
    lcd_wr_regno( 0x47 );  // fsm_hfp_o[5:0]
    lcd_wr_data( 0x2a );
    lcd_wr_regno( 0x48 );  // fsm_hbp_e[5:0]
    lcd_wr_data( 0x0a );
    lcd_wr_regno( 0x49 );  // fsm_hfp_e[5:0]
    lcd_wr_data( 0x1a );
    lcd_wr_regno( 0x44 );  // VBP
    lcd_wr_data( 0x15 );
    lcd_wr_regno( 0x45 );  // VFP
    ...
    // lcd_wr_regno(0xf9);
    // lcd_wr_data(0x20);
    lcd_wr_regno( 0x56 );  // src_ld_wd[1:0] src_ld_st[5:0]
    lcd_wr_data( 0x43 );
    lcd_wr_regno( 0x57 );  // pn_cs_en src_cs_st[5:0]
    lcd_wr_data( 0x42 );
    lcd_wr_regno( 0x58 );  // src_cs_p_wd[6:0]
    lcd_wr_data( 0x3c );
    lcd_wr_regno( 0x59 );  // src_cs_n_wd[6:0]
    lcd_wr_data( 0x64 );
    lcd_wr_regno( 0x5a );  // src_pchg_st_o[6:0]
    lcd_wr_data( 0x41 );
    lcd_wr_regno( 0x5b );  // src_pchg_wd_o[6:0]
    lcd_wr_data( 0x3C );
    lcd_wr_regno( 0x5c );  // src_pchg_st_e[6:0]
    lcd_wr_data( 0x02 );
    lcd_wr_regno( 0x5d );  // src_pchg_wd_e[6:0]
    lcd_wr_data( 0x3c );
    lcd_wr_regno( 0x5e );  // src_pol_sw[7:0]
    lcd_wr_data( 0x1f );
    lcd_wr_regno( 0x60 );  // src_op_st_o[7:0]
    lcd_wr_data( 0x80 );
    lcd_wr_regno( 0x61 );  // src_op_st_e[7:0]
    lcd_wr_data( 0x3f );
    lcd_wr_regno( 0x62 );  // src_op_ed_o[9:8] src_op_ed_e[9:8]
    lcd_wr_data( 0x21 );
    lcd_wr_regno( 0x63 );  // src_op_ed_o[7:0]
    lcd_wr_data( 0x07 );
    lcd_wr_regno( 0x64 );  // src_op_ed_e[7:0]
    lcd_wr_data( 0xe0 );
    lcd_wr_regno( 0x65 );  // chopper
    lcd_wr_data( 0x02 );
    lcd_wr_regno( 0xca );  // avdd_mux_st_o[7:0]
    lcd_wr_data( 0x20 );
    lcd_wr_regno( 0xcb );  // avdd_mux_ed_o[7:0]
    lcd_wr_data( 0x52 );
    lcd_wr_regno( 0xcc );  // avdd_mux_st_e[7:0]
    lcd_wr_data( 0x10 );
    lcd_wr_regno( 0xcD );  // avdd_mux_ed_e[7:0]
    lcd_wr_data( 0x42 );
    lcd_wr_regno( 0xD0 );  // avcl_mux_st_o[7:0]
    lcd_wr_data( 0x20 );
    lcd_wr_regno( 0xD1 );  // avcl_mux_ed_o[7:0]
    lcd_wr_data( 0x52 );
    lcd_wr_regno( 0xD2 );  // avcl_mux_st_e[7:0]
    lcd_wr_data( 0x10 );
    lcd_wr_regno( 0xD3 );  // avcl_mux_ed_e[7:0]
    lcd_wr_data( 0x42 );
    lcd_wr_regno( 0xD4 );  // vgh_mux_st[7:0]
    lcd_wr_data( 0x0a );
    lcd_wr_regno( 0xD5 );  // vgh_mux_ed[7:0]
    lcd_wr_data( 0x32 );
    // gammma boe4.3
    lcd_wr_regno( 0x80 );  // gam_vrp0
    lcd_wr_data( 0x00 );
    lcd_wr_regno( 0xA0 );  // gam_VRN0
    lcd_wr_data( 0x00 );
    lcd_wr_regno( 0x81 );  // gam_vrp1
    lcd_wr_data( 0x06 );   // 07
    lcd_wr_regno( 0xA1 );  // gam_VRN1
    lcd_wr_data( 0x08 );   // 06
    lcd_wr_regno( 0x82 );  // gam_vrp2
    lcd_wr_data( 0x03 );   // 02
    lcd_wr_regno( 0xA2 );  // gam_VRN2
    lcd_wr_data( 0x03 );   // 01
    lcd_wr_regno( 0x86 );  // gam_prp0
    lcd_wr_data( 0x14 );   // 11
    lcd_wr_regno( 0xA6 );  // gam_PRN0
    lcd_wr_data( 0x14 );   // 10
    lcd_wr_regno( 0x87 );  // gam_prp1
    lcd_wr_data( 0x2C );   // 27
    lcd_wr_regno( 0xA7 );  // gam_PRN1
    lcd_wr_data( 0x26 );   // 27
    lcd_wr_regno( 0x83 );  // gam_vrp3
    lcd_wr_data( 0x37 );
    lcd_wr_regno( 0xA3 );  // gam_VRN3
    lcd_wr_data( 0x37 );
    lcd_wr_regno( 0x84 );  // gam_vrp4
    lcd_wr_data( 0x35 );
    lcd_wr_regno( 0xA4 );  // gam_VRN4
    lcd_wr_data( 0x35 );
    lcd_wr_regno( 0x85 );  // gam_vrp5
    lcd_wr_data( 0x3f );
    lcd_wr_regno( 0xA5 );  // gam_VRN5
    lcd_wr_data( 0x3f );
    lcd_wr_regno( 0x88 );  // gam_pkp0
    lcd_wr_data( 0x0A );   // 0b
    lcd_wr_regno( 0xA8 );  // gam_PKN0
    lcd_wr_data( 0x0A );   // 0b
    lcd_wr_regno( 0x89 );  // gam_pkp1
    lcd_wr_data( 0x13 );   // 14
    lcd_wr_regno( 0xA9 );  // gam_PKN1
    lcd_wr_data( 0x12 );   // 13
    lcd_wr_regno( 0x8a );  // gam_pkp2
    lcd_wr_data( 0x18 );   // 1a
    lcd_wr_regno( 0xAa );  // gam_PKN2
    lcd_wr_data( 0x19 );   // 1a
    lcd_wr_regno( 0x8b );  // gam_PKP3
    lcd_wr_data( 0x0a );
    lcd_wr_regno( 0xAb );  // gam_PKN3
    lcd_wr_data( 0x0a );
    lcd_wr_regno( 0x8c );  // gam_PKP4
    lcd_wr_data( 0x17 );   // 14
    lcd_wr_regno( 0xAc );  // gam_PKN4
    lcd_wr_data( 0x0B );   // 08
    lcd_wr_regno( 0x8d );  // gam_PKP5
    lcd_wr_data( 0x1A );   // 17
    lcd_wr_regno( 0xAd );  // gam_PKN5
    lcd_wr_data( 0x09 );   // 07
    lcd_wr_regno( 0x8e );  // gam_PKP6
    lcd_wr_data( 0x1A );   // 16 //16
    lcd_wr_regno( 0xAe );  // gam_PKN6
    lcd_wr_data( 0x08 );   // 06 //13
    lcd_wr_regno( 0x8f );  // gam_PKP7
    lcd_wr_data( 0x1F );   // 1B
    lcd_wr_regno( 0xAf );  // gam_PKN7
    lcd_wr_data( 0x00 );   // 07
    lcd_wr_regno( 0x90 );  // gam_PKP8
    lcd_wr_data( 0x08 );   // 04
    lcd_wr_regno( 0xB0 );  // gam_PKN8
    lcd_wr_data( 0x00 );   // 04
    lcd_wr_regno( 0x91 );  // gam_PKP9
    lcd_wr_data( 0x10 );   // 0A
    lcd_wr_regno( 0xB1 );  // gam_PKN9
    lcd_wr_data( 0x06 );   // 0A

    lcd_wr_regno( 0x92 );  // gam_PKP10
    lcd_wr_data( 0x19 );   // 16
    lcd_wr_regno( 0xB2 );  // gam_PKN10
    lcd_wr_data( 0x15 );   // 15
    lcd_wr_regno( 0xff );
    lcd_wr_data( 0x00 );
    lcd_wr_regno( 0x11 );
