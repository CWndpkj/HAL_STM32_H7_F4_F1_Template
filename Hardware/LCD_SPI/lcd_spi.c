...
HAL_StatusTypeDef LCD_SPI_Transmit( SPI_HandleTypeDef* hspi3, uint16_t pData, uint32_t Size );
HAL_StatusTypeDef LCD_SPI_TransmitBuffer( SPI_HandleTypeDef* hspi3, uint16_t* pData, uint32_t Size );

/****************************************************************************************************************************************
 *	函 数 名:	HAL_SPI_MspInit
 *	入口参数:	hspi3 - SPI_HandleTypeDef定义的变量，即表示定义的 SPI 句柄
 *	函数功能:	初始化 SPI 引脚
 ****************************************************************************************************************************************/

void HAL_SPI_MspInit( SPI_HandleTypeDef* hspi3 )
{
    GPIO_InitTypeDef GPIO_InitStruct = { 0 };
    if ( hspi3->Instance == LCD_SPI ) {
        GPIO_SPI_CLK_ENABLE;     // 使能SPI相关GPIO时钟
        SPI_CLK_ENABLE;          // 使能SPI时钟
        GPIO_LCD_BK_CLK_ENABLE;  // 使能 背光        引脚时钟
        GPIO_LCD_DC_CLK_ENABLE;  // 使能 数据指令选择 引脚时钟
        GPIO_LCD_CS_CLK_ENABLE;  // 使能 片选(CS) 引脚时钟
        /******************************************************
        ...
        *******************************************************/
        ...
        GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;  // 最高速度等级
        GPIO_InitStruct.Pin   = GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_7;
        HAL_GPIO_Init( GPIOA, &GPIO_InitStruct );

        // 初始化 背光 引脚
        GPIO_InitStruct.Pin   = LCD_BK_PIN;              // 背光 引脚
        GPIO_InitStruct.Mode  = GPIO_MODE_OUTPUT_PP;     // 推挽输出模式
        GPIO_InitStruct.Pull  = GPIO_NOPULL;             // 无上下拉
        GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;     // 速度等级低
        HAL_GPIO_Init( LCD_BK_PORT, &GPIO_InitStruct );  // 初始化
        ...
        // 初始化 片选(CS) 引脚
        GPIO_InitStruct.Pin   = LCD_CS_PIN;              // 数据指令选择 引脚
        GPIO_InitStruct.Mode  = GPIO_MODE_OUTPUT_PP;     // 推挽输出模式
        GPIO_InitStruct.Pull  = GPIO_NOPULL;             // 无上下拉
        GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;     // 速度等级低
...
{
    hspi3.Instance         = LCD_SPI;              //	使用SPI5
    hspi3.Init.Mode        = SPI_MODE_MASTER;      //	主机模式
    hspi3.Init.Direction   = SPI_DIRECTION_1LINE;  //	单线
    hspi3.Init.DataSize    = SPI_DATASIZE_8BIT;    //	8位数据宽度
    hspi3.Init.CLKPolarity = SPI_POLARITY_LOW;     //	CLK空闲时保持低电平
    hspi3.Init.CLKPhase    = SPI_PHASE_1EDGE;      //	数据在CLK第一个边沿有效
    hspi3.Init.NSS         = SPI_NSS_SOFT;         //	使用软件片选

    // SPI的内核时钟设置为120M，再经过2分频得到 60M 的SCK驱动时钟
    hspi3.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_64;

    hspi3.Init.FirstBit       = SPI_FIRSTBIT_MSB;            //	高位在先
    hspi3.Init.TIMode         = SPI_TIMODE_DISABLE;          //	禁止TI模式
    hspi3.Init.CRCCalculation = SPI_CRCCALCULATION_DISABLE;  //	禁止CRC
    hspi3.Init.CRCPolynomial  = 7;                           // CRC校验项，这里用不到
    HAL_SPI_Init( &hspi3 );
}
...
void LCD_WriteCommand( uint8_t lcd_command )
{
    ...
    HAL_SPI_Transmit( &hspi3, &lcd_command, 1, 1000 );  // 启动SPI传输
    LCD_CS_HIGH;
}
...
void LCD_WriteData_8bit( uint8_t lcd_data )
{
    ...
    HAL_SPI_Transmit( &hspi3, &lcd_data, 1, 1000 );  // 启动SPI传输
    LCD_CS_HIGH;
}
...
void LCD_WriteData_16bit( uint16_t lcd_data )
{
    ...
    HAL_SPI_Transmit( &hspi3, lcd_data_buff, 2, 1000 );  // 启动SPI传输
    LCD_CS_HIGH;
}
/****************************************************************************************************************************************
 *	函 数 名: LCD_WriteBuff
...
void LCD_WriteBuff( uint16_t* DataBuff, uint16_t DataSize )
{
    ...
    hspi3.Init.DataSize = SPI_DATASIZE_16BIT;  //	16位数据宽度
    HAL_SPI_Init( &hspi3 );

    HAL_SPI_Transmit( &hspi3, ( uint8_t* )DataBuff, DataSize, 1000 );  // 启动SPI传输
...
void LCD_Init( void )
{
    ...
    // 以下进行一些驱动的默认设置
    LCD_SetDirection( Direction_V );  // 置显示方向
...
void LCD_Clear()
{
    ...
    LCD_SPI_Transmit( &hspi3, LCD.BackColor, LCD.Width * LCD.Height );  // 启动传输
...
void LCD_ClearRect( uint16_t x, uint16_t y, uint16_t width, uint16_t height )
{
    ...
    LCD_SPI_Transmit( &hspi3, LCD.BackColor, width * height );  // 启动传输
...
void LCD_DisplayCHString( uint16_t x, uint16_t y, char* p )
{
    while ( ( x < LCD.Width ) && ( *p != 0 ) )  // 判断显示坐标是否超出显示区域并且字符是否为空字符
    {
        LCD_DisplayChinese( x, y, p );
        x += LCD_CHFonts->Width;  // 显示下一个字符
        p += 3  ;  // 取下一个字符地址
    }
}
/****************************************************************************************************************************************
...
void LCD_FillRect( uint16_t x, uint16_t y, uint16_t width, uint16_t height )
{
    ...
    LCD_SPI_Transmit( &hspi3, LCD.Color, width * height );
...
void LCD_CopyBuffer( uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t* DataBuff )
{
    ...
    LCD_SPI_TransmitBuffer( &hspi3, DataBuff, width * height );

    //	HAL_SPI_Transmit(&hspi35, (uint8_t *)DataBuff, (x2-x1+1) * (y2-y1+1), 1000) ;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef LCD_SPI_WaitOnFlagUntilTimeout( SPI_HandleTypeDef* hspi3, uint32_t Flag, FlagStatus Status, uint32_t Tickstart, uint32_t Timeout )
{
    /* Wait until flag is set */
    while ( ( __HAL_SPI_GET_FLAG( hspi3, Flag ) ? SET : RESET ) == Status ) {
 ...
 */
void LCD_SPI_CloseTransfer( SPI_HandleTypeDef* hspi3 )
{
    ...
    /* Disable ITs */
    __HAL_SPI_DISABLE_IT( hspi3, ( SPI_IT_TXE | SPI_IT_RXNE | SPI_IT_ERR ) );
    ...
    if ( hspi3->State != HAL_SPI_STATE_BUSY_TX ) {
        if ( ( itflag & SPI_FLAG_OVR ) != 0UL ) {
            SET_BIT( hspi3->ErrorCode, HAL_SPI_ERROR_OVR );
    ...
    if ( ( itflag & SPI_FLAG_MODF ) != 0UL ) {
        SET_BIT( hspi3->ErrorCode, HAL_SPI_ERROR_MODF );
        __HAL_SPI_CLEAR_MODFFLAG( hspi3 );
    }

    hspi3->TxXferCount = ( uint16_t )0UL;
    hspi3->RxXferCount = ( uint16_t )0UL;
}
...
HAL_StatusTypeDef LCD_SPI_Transmit( SPI_HandleTypeDef* hspi3, uint16_t pData, uint32_t Size )
{
    ...
    assert_param( IS_SPI_DIRECTION_2LINES_OR_1LINE_2LINES_TXONLY( hspi3->Init.Direction ) );
    ...
    hspi3->State     = HAL_SPI_STATE_BUSY_TX;
    ...
    hspi3->pRxBuffPtr  = NULL;
    hspi3->RxXferSize  = ( uint16_t )0UL;
    hspi3->RxXferCount = ( uint16_t )0UL;
    hspi3->TxISR       = NULL;
    hspi3->RxISR       = NULL;

    /* Configure communication direction : 1Line */
    if ( hspi3->Init.Direction == SPI_DIRECTION_1LINE ) {
        SPI_1LINE_TX( hspi3 );
    }
    ...
    while ( LCD_TxDataCount > 0UL ) {
        ...
        if ( __HAL_SPI_GET_FLAG( hspi3, SPI_FLAG_TXE ) ) {
            if ( hspi3->TxXferCount > 1UL ) {
                hspi3->Instance->DR = ( uint32_t )LCD_pData_32bit;
                LCD_TxDataCount -= ( uint16_t )2UL;
            }
            else {
                hspi3->Instance->DR = ( uint16_t )pData;
                LCD_TxDataCount--;
            }
        }
        else {
            ...
            if ( ( ( ( HAL_GetTick() - tickstart ) >= Timeout ) && ( Timeout != HAL_MAX_DELAY ) ) || ( Timeout == 0U ) ) {
                ...
                LCD_SPI_CloseTransfer( hspi3 ); /* Call standard close procedure with error check */
                goto error;
            }
        }
    }
    ...
    while ( LCD_TxDataCount > 0UL ) {
        ...
        if ( __HAL_SPI_GET_FLAG( hspi3, SPI_FLAG_TXE ) ) {
            if ( ( LCD_TxDataCount > 1UL ) ) {
                hspi3->Instance->DR = *( ( uint16_t* )pData );
                pData += 2;
                hspi3->pTxBuffPtr += sizeof( uint16_t );
                LCD_TxDataCount -= 2;
            }
        }
        else {
