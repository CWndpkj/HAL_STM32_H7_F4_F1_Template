...
#define LCD_SPI SPI1                                      // SPI局部宏，方便修改和移植
#define GPIO_SPI_CLK_ENABLE __HAL_RCC_GPIOA_CLK_ENABLE()  // 背光 GPIO时钟
#define SPI_CLK_ENABLE __HAL_RCC_SPI1_CLK_ENABLE();       // SPI1时钟
/*--------------------------------------------- LCD其它引脚 -----------------------------------------------*/

#define LCD_BK_PIN GPIO_PIN_5                                // 背光  引脚
#define LCD_BK_PORT GPIOB                                    // 背光 GPIO端口
#define GPIO_LCD_BK_CLK_ENABLE __HAL_RCC_GPIOB_CLK_ENABLE()  // 背光 GPIO时钟
#define LCD_BK_OFF HAL_GPIO_WritePin( LCD_BK_PORT, LCD_BK_PIN, GPIO_PIN_RESET );
#define LCD_BK_ON HAL_GPIO_WritePin( LCD_BK_PORT, LCD_BK_PIN, GPIO_PIN_SET );

#define LCD_DC_PIN GPIO_PIN_14                                                        // 数据指令选择  引脚
#define LCD_DC_PORT GPIOB                                                             // 数据指令选择  GPIO端口
#define GPIO_LCD_DC_CLK_ENABLE __HAL_RCC_GPIOB_CLK_ENABLE()                           // 数据指令选择  GPIO时钟
...
#define LCD_CS_PIN GPIO_PIN_13                                                    // 数据指令选择  引脚
#define LCD_CS_PORT GPIOB                                                         // 数据指令选择  GPIO端口
#define GPIO_LCD_CS_CLK_ENABLE __HAL_RCC_GPIOB_CLK_ENABLE()                       // 数据指令选择  GPIO时钟
#define LCD_CS_LOW HAL_GPIO_WritePin( LCD_DC_PORT, LCD_DC_PIN, GPIO_PIN_RESET );  // 低电平，指令传输
#define LCD_CS_HIGH HAL_GPIO_WritePin( LCD_DC_PORT, LCD_DC_PIN, GPIO_PIN_SET );   // 高电平，数据传输
