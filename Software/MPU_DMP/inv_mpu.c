...
#if defined STM32F407xx  // 在编译器中定义 STM32F10X_MD 则使用这段,以下的系统平台将不会用到
...
#define i2c_write( a, b, c, d ) HAL_I2C_Mem_Write( &hi2c3, a<<1, b, 1, d, c, HAL_MAX_DELAY )
#define i2c_read( a, b, c, d ) HAL_I2C_Mem_Read( &hi2c3, a<<1, b, 1, d, c, HAL_MAX_DELAY )
#define delay_ms delay_ms
#define get_ms(count) do { unsigned long temp = HAL_GetTick(); *count = temp; } while(0)
// 中断回调函数
...
const struct hw_s hw = { .addr        = MPU_Device_Adrr,
                         .max_fifo    = 1024,
                         .num_reg     = 128,
                         .temp_sens   = 321,
                         .temp_offset = 0,
