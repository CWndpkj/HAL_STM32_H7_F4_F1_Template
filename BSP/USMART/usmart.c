...
/* 系统命令 */
char* sys_cmd_tab[] = {
    "?", "help", "list", "id", "hex", "dec", "runtime", "data_transfer_mode",
};
...
uint8_t usmart_sys_cmd_exe( char* str )
{
    ...
    case 1: /* 帮助指令 */
        ...
        USMART_PRINTF( "data_transfer_mode:长数据传输模式,中途退出请输入'+++'\r\n\n" );
        USMART_PRINTF( "dec:        参数10进制显示,后跟空格+数字即执行进制转换\r\n\n" );
    ...
    case 6: /* runtime指令,设置是否显示函数执行时间 */
        ...
        USMART_PRINTF( "\r\n" );
        break;
    case 7:  // data_transfer_mode
        data_rcv_handler.last_transfer_timestamp = HAL_GetTick();
        usmart_dev.mode_flag                     = 1;
        g_usart_rx_sta |= 0x2000;  // 更改接收模式
        HAL_UART_AbortReceive_IT( &huart_debug );
        HAL_UART_Receive_DMA( &huart_debug, g_rx_buffer, RXBUFFERSIZE );
        USMART_PRINTF( "data transfer start>>>\r\n" );
        break;
    default: /* 非法指令 */
...
void usmart_exe( void )
{
    ...
    for ( i = 0; i < pnum; i++ ) /* 输出参数 */
    {
        ...
        else /* 参数是数字 */
        {
            ...
            if ( usmart_dev.sptype == SP_TYPE_DEC ) {
                ...
                USMART_PRINTF( "0x%lX", ( unsigned long )temp[ i ] ); /* 16进制参数显示 */
            }
        }
    ...
    case 6: /* 有6个参数 */
        res = ( *( uint32_t( * )() )usmart_dev.funs[ id ].func )( temp[ 0 ], temp[ 1 ], temp[ 2 ], temp[ 3 ], temp[ 4 ], temp[ 5 ] );
        break;

    case 7: /* 有7个参数 */
        res = ( *( uint32_t( * )() )usmart_dev.funs[ id ].func )( temp[ 0 ], temp[ 1 ], temp[ 2 ], temp[ 3 ], temp[ 4 ], temp[ 5 ], temp[ 6 ] );
        break;

    case 8: /* 有8个参数 */
        res = ( *( uint32_t( * )() )usmart_dev.funs[ id ].func )( temp[ 0 ], temp[ 1 ], temp[ 2 ], temp[ 3 ], temp[ 4 ], temp[ 5 ], temp[ 6 ], temp[ 7 ] );
        break;

    case 9: /* 有9个参数 */
        res = ( *( uint32_t( * )() )usmart_dev.funs[ id ].func )( temp[ 0 ], temp[ 1 ], temp[ 2 ], temp[ 3 ], temp[ 4 ], temp[ 5 ], temp[ 6 ], temp[ 7 ], temp[ 8 ] );
        break;

    case 10: /* 有10个参数 */
        res = ( *( uint32_t( * )() )usmart_dev.funs[ id ].func )( temp[ 0 ], temp[ 1 ], temp[ 2 ], temp[ 3 ], temp[ 4 ], temp[ 5 ], temp[ 6 ], temp[ 7 ], temp[ 8 ], temp[ 9 ] );
        break;
    }
    ...
    if ( rval == 1 ) /* 需要返回值. */
    {
        if ( usmart_dev.sptype == SP_TYPE_DEC )
            ...
            USMART_PRINTF( "=0x%lX;\r\n", ( unsigned long )res ); /* 输出执行结果(16进制参数显示) */
    }
    else
        USMART_PRINTF( ";\r\n" ); /* 不需要返回值,直接输出结束 */
    ...
    {
        USMART_PRINTF( "Function Run Time:%ld.%1ldms\r\n", ( unsigned long int )usmart_dev.runtime / 10, ( unsigned long int )usmart_dev.runtime % 10 ); /* 打印函数执行时间 */
    }
}
...
{
    // 查看是否应该超时退出数据传输模式
    if ( ( HAL_GetTick() - data_rcv_handler.last_transfer_timestamp > 10000 ) && usmart_dev.mode_flag == 1 ) {
        USMART_PRINTF( "exit data rcv mode:current->%lx,last->%lx\r\n", ( unsigned long )HAL_GetTick(), ( unsigned long )data_rcv_handler.last_transfer_timestamp );
        HAL_UART_DMAStop( &huart_debug );
        HAL_UART_Receive_IT( &huart_debug, ( uint8_t* )g_rx_buffer, 1 );
        usmart_dev.mode_flag = 0;
        g_usart_rx_sta &= 0xDFFF;  // 更改接收模式
    }
