...
static const char* TAG = "lcd_8080";
...
void lcd_scan_dir( uint8_t dir )
{
    ...
    if ( ( lcddev.dir == 1 && lcddev.id != 0x1963 ) || ( lcddev.dir == 0 && lcddev.id == 0x1963 ) || lcddev.id == 0x6201 ) {
        switch ( dir ) /* 方向转换 */
        {
        case 0:
            dir = 6;
    ...
    if ( lcddev.id != 0x1963 && lcddev.id != 0x6201 ) /* 1963不做坐标处理 */
    {
        if ( regval & 0x20 ) {
...
void lcd_display_dir( uint8_t dir )
{
    ...
    {
        DFT_SCAN_DIR  = R2L_D2U;  // 横显-设定显示方向
...
void lcd_init( void )
{
    ...
    /* FSMC写时序控制寄存器 */
    fsmc_write_handle.AddressSetupTime = 3;            /* 地址建立时间(ADDSET)为9个fsmc_ker_ck=6*9=54ns */
    fsmc_write_handle.AddressHoldTime  = 0x00;         /* 地址保持时间(ADDHLD) 模式A是没有用到 */
    fsmc_write_handle.DataSetupTime    = 60;           /* 数据保存时间(DATAST)为9个fsmc_ker_ck=6*9=54ns */
    ...
    if ( lcddev.id != 0x9341 ) /* 不是 9341 , 尝试看看是不是 ST7789 */
    {
        ...
        if ( lcddev.id == 0x3030 )  // st6201
        {
            lcddev.id = 0x6201;
        }
        if ( lcddev.id != 0x7789 && lcddev.id != 0x6201 ) /* 也不是ST7789, 尝试是不是 NT35310 */
        {
            lcd_wr_regno( 0xD4 );
            lcddev.id = lcd_rd_data(); /* dummy read */
...
void lcd_fill_rect_with_buffer( uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint16_t* pbuffer )
{
    ...
    for ( int i = 0; i < xlen * ylen; i++ ) {
        LCD->LCD_RAM = pbuffer[ i ];
    }
    // memcpy( ( void* )( &( LCD->LCD_RAM ) + xlen * ( ey - sy ) + sx ), pbuffer, sizeof( uint16_t ) * ( xlen * ylen ) );
}
...
void lcd_color_fill( uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint16_t* color )
{
    ...
    for ( i = 0; i < height; i++ ) {
        if ( lcddev.id == 0x6201 ) {
            lcd_set_window( sx, sy + i, width, 1 );
        }
        else {
            lcd_set_cursor( sx, sy + i ); /* 设置光标位置 */
        }
...
void lcd_show_char( uint16_t x, uint16_t y, char chr, uint8_t size, uint8_t mode, uint16_t color )
{
    ...
    switch ( size ) {
    case 12:
        pfont = ( uint8_t* )asc2_1206[ ( unsigned char )chr ]; /* 调用1206字体 */
        break;

    case 16:
        pfont = ( uint8_t* )asc2_1608[ ( unsigned char )chr ]; /* 调用1608字体 */
        break;

    case 24:
        pfont = ( uint8_t* )asc2_2412[ ( unsigned char )chr ]; /* 调用2412字体 */
        break;

    case 32:
        pfont = ( uint8_t* )asc2_3216[ ( unsigned char )chr ]; /* 调用3216字体 */
        break;
